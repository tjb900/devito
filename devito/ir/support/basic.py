...
class Vector(tuple):
    ...
    def __lt__(self, other):
        try:
            diff = [int(i) for i in self.distance(other)]
        except TypeError:
...
class Access(IterationInstance):
    ...
    def __repr__(self):
        return "Access<%s,`%s`,[%s]>" % (self.function.name, self.mode,
...
class Dependence(object):
    ...
    def is_indirect(self):
        ...
        False otherwise."""
        return all(i == j for i, j in zip(self.source.index_mode, self.sink.index_mode))\
            and any(i == 'indirect' for i in self.source.index_mode)
