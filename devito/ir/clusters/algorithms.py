    ...
    for c in clusters:
        ...
        for candidate in reversed(list(processed)):
            ...
            if candidate.stencil == c.stencil and\
                ...
                # /c/ will be fused into /candidate/. However, all
                # fusion-induced anti dependences need to be removable.
                # If that's the case, these dependences are eliminated
                # by so called "index bumping and array contraction",
                # which transforms array accesses into scalars
