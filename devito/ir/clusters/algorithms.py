    ...
    for c in clusters:
        ...
        for candidate in reversed(list(processed)):
            ...
            if candidate.stencil == c.stencil:
                fake = scope.d_flow.independent() - scope.d_flow.indirect()
                fdeps = [i.function for i in true + fake]
                if all(is_local(i, candidate, c, clusters) for i in fdeps):
                    ...
                    merged = _merge(*bump_and_contract(fdeps, candidate, c))
